...
/**
 * Fills its parent container in the hero. Pointer-transparent and aria-hidden:
 * the site loses zero information if this never mounts.
 */
export default function SceneRoot({ scroll, theme }) {
  const [visible, setVisible] = useState(false)

  // Fade in over 700ms once the first frame is on screen, so the hero text is
  // never competing with a pop-in.
  useEffect(() => {
  ...
  const count = isCoarse ? 2744 : 4096 // 14³ on touch, 16³ on desktop

  return (
    <div className="scene" aria-hidden="true" style={{ opacity: visible ? 1 : 0 }}>
      <Canvas
